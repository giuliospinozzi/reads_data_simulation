...
def generateAssociationFiles (List_of_Simulation_RUNs, unique_bed, ref_unique_bed, assFile_name = 'auto', assFile_path = 'current_location'):
	...
	for Simulation_RUN in List_of_Simulation_RUNs:
		...
		### assFile
		bedFile_name = unique_bed # bedFile_name in place of 'barcode join', 'LAM_id' and 'complete name of LAM'
		...
		### REFassFile
		if (reference_files == True):
			REFbedFile_name = ref_unique_bed # REFbedFile_name in place of 'barcode join', 'LAM_id' and 'complete name of LAM'
...
def exportDataToDB (PYTHON_TOOL, bedFile_path_and_name, associationFile_path_and_name, patient, pool, tag, DB, dbschema, dbtable):
	...
	### Prepare command
	command = ['python', PYTHON_TOOL, '-b', bedFile_path_and_name, '-a', associationFile_path_and_name, '--db', DB, '--patient', patient, '--pool', pool, '--tag', tag, '--dbschema', dbschema, '--dbtable', dbtable]
